...
while(True):
    ...
    if(ret):
        ...
        reduced_frame = cv2.resize(raw_frame, (0,0), fx=0.5, fy=0.5) 
        flipped_frame = cv2.flip(reduced_frame,1)
        grayed_frame = cv2.cvtColor(flipped_frame, cv2.COLOR_BGR2GRAY)
        ...
        for (x,y,w,h) in faces:
            ...
            detected_face = flipped_frame[int(y):int(y+h), int(x):int(x+w)] #crop detected face
